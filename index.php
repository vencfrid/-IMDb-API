	...
	<div id="div1">
		<table id="table1">
			<tr>
				<td id="TdTable1">			
					Vyhledej v OMDb
					<form name="sign" id="sign" action="OMDb_inc.php" method="GET">
			...
			//var_dump($OMDbData);	
				if(isset($OMDbData['Search']))					// zobrazení výsledků (max 10)
				{
					...
					foreach ($OMDbData['Search'] as $key => $value) 
					{
						echo '<tr>';
							echo '<td>';
								echo '<a href="https://www.imdb.com/title/'.$value['imdbID'].'">'.$value['Title'].'</a>';
								//echo $value['Title'];
									echo'<br>';
								echo '<a href="https://www.imdb.com/title/'.$value['imdbID'].'"><img src="'.$value['Poster'].'"width="500" height="600"></a>';
								//echo $value['Poster'];
							echo '</td>';
						echo '</tr>';
						//var_dump($OMDbData);					
